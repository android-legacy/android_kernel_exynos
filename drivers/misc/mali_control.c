...
static ssize_t gpu_clock_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	return sprintf(buf,
			"Step0: %d\nStep1: %d\nStep2: %d\nStep3: %d\nStep4: %d\n",
			gm_mali_dvfs[0].clock, gm_mali_dvfs[1].clock,
			gm_mali_dvfs[2].clock, gm_mali_dvfs[3].clock,
			gm_mali_dvfs[4].clock);
}

static ssize_t gpu_clock_store(struct device *dev,
			...
			const char *buf, size_t count)
{
	...
	if ((ret=sscanf(buf, "%d %d %d %d %d", &g[0], &g[1],
			&g[2], &g[3], &g[4]))!=5)
		return -EINVAL;
...
static ssize_t gpu_voltage_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	return sprintf(buf,
			"Step1: %d\nStep2: %d\nStep3: %d\nStep4: %d\nStep5: %d\n",
			gm_mali_dvfs[0].vol, gm_mali_dvfs[1].vol,
			gm_mali_dvfs[2].vol, gm_mali_dvfs[3].vol,
			gm_mali_dvfs[4].vol);
}

static ssize_t gpu_voltage_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%d %d %d %d %d", &gv[0], &gv[1],
			&gv[2], &gv[3], &gv[4]);
	if (ret != 5) {
		return -EINVAL;
	}
...
static DEVICE_ATTR(voltage_control, S_IRUGO |
		S_IWUGO, gpu_voltage_show, gpu_voltage_store);
static DEVICE_ATTR(clock_control, S_IRUGO |
		S_IWUGO, gpu_clock_show, gpu_clock_store);
...
int register_mali_control(void)
{
	...
	gm_mali_dvfs_all = (mali_dvfs_table *)
			kallsyms_lookup_name("mali_dvfs_all");
	gm_mali_dvfs_threshold = (mali_dvfs_threshold_table *)
			kallsyms_lookup_name("mali_dvfs_threshold");
	gm_mali_dvfs_control = (int *)
			kallsyms_lookup_name("mali_dvfs_control");
	...
	if (sysfs_create_group(&malicontrol_device.this_device->kobj,
			&malicontrol_group) < 0) {
...
{
	sysfs_remove_group(&malicontrol_device.this_device->kobj,
			&malicontrol_group);
